...
export const useTimer = () => {
  ...
  const playCompletionSound = useCallback(async () => {
    try {
      ...
      const { sound } = await Audio.Sound.createAsync(TIMER_SOUND, {
        shouldPlay: true,
        isLooping: true,
        volume: 1.0,
      });
  ...
  const createSession = useCallback(async () => {
    ...
    if (!online) {
      // Queue session creation — sessionIdRef stays null until sync
      await syncQueue.push("session_save", payload);
      return;
    }
        ...
        if (newElapsed >= dur) {
          ...
          if (sessionIdRef.current) {
            ...
            if (online) {
              ...
              await syncQueue.push("session_save", {

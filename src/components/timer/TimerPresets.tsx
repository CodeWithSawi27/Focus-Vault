...
export const TimerPresets = ({
  ...
  const [defaultPreset, setDefaultPreset] = useState<number>(
    presets[0].seconds,
  );
  ...
  const styles = useMemo(
    () =>
      StyleSheet.create({
        ...
        customUnit: { ...Typography.subhead, color: colors.text.tertiary },
        customConfirm: {
          width: 32,
          height: 32,
          borderRadius: 16,
  ...
  return (
    <View style={styles.wrapper}>
      <View style={styles.chips}>
          ...
          return (
            ...
            >
              <TouchableOpacity
                ...
                onLongPress={() => handleLongPress(preset.seconds)}
                delayLongPress={400}
                disabled={disabled}
                activeOpacity={0.7}
                style={[
              ...
              >
                {isDefault && (
                  <Star
                    size={10}
                    color={active ? colors.text.inverse : colors.accent.orange}
                    fill={active ? colors.text.inverse : colors.accent.orange}
                    strokeWidth={2}
                  />
                )}
                <Text
        ...
        <TouchableOpacity
          onPress={() => {
            if (!disabled) setCustomMode(true);
          }}
          disabled={disabled}
          activeOpacity={0.7}
          style={[
